...
const USER_AGENT =
	"Mozilla/5.0 (compatible; EchoBot/1.0; academic research; +https://github.com/katchaa/crawler)";
...
function extractText($, source) {
	...
	for (const sel of candidates) {
		...
		if (el.length) {
			...
			el.find(
				"nav, aside, .related, .advertisement, .ad, script, style, figure, figcaption, .social-share",
			).remove();
			const paragraphs = el
				.find("p")
				.map((_, p) => $(p).text().trim())
...
async function collect(options = {}) {
	const { maxPerSource = 25, daysCutoff = 7 } = options;
			...
			for (const item of items) {
				...
				if (article) {
					...
					process.stdout.write(
						`  [${collected}/${maxPerSource}] OK: ${article.title.substring(0, 60)}…\n`,
					);
				}
			}
		}
